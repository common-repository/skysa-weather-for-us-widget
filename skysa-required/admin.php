...
function SkysaApps_Admin_CreateDBTable($table_name,$app_id,$options,$window){
    ...
    if($options){
        foreach($options as $field => $valuelist){
            if($valuelist['type'] == 'selectbox'){
                ...
                $updateArray[$field] = $wpdb->prepare($optionvalues[0]);
            }
            else if($valuelist['type'] == 'checkbox'){
                $updateArray[$field] = '';
            }
            else{
                $updateArray[$field] = $wpdb->prepare($valuelist['value']);
            }
        }
        unset($valuelist);
    }
    if($window){
        foreach($window as $field => $value){                              
            $updateArray[$field] = $wpdb->prepare($value);
        }
        unset($value);
    }
    $wpdb->insert( 
...
function SkysaApps_Admin_AppPage(){
    ...
    $table_name=$wpdb->prefix . "skysa_apps";$updateArray = array();
    $curRec = $wpdb->get_row(
        "
                        ...
                        if ( isset($_POST['submit'])) {
                            ...
                            foreach($options as $field => $valuelist){
                                    if(isset($_POST[$field])){
                                        $updateArray[$field] = $wpdb->prepare($_POST[$field]);
                                    }
                            }
                            unset($valuelist);
                            if(isset($_POST['align'])){
                                $updateArray['align'] = $wpdb->prepare($_POST['align']);
                            }
                            $wpdb->update( 
	                            $table_name, 
                                $updateArray,
                        ...
                        if(is_array($manage)){
                            ...
                            if ( isset($_POST['submit'])) {
                                ...
                                if(isset($_POST['sk_recid'])){
                                    $updateArray['sk_recid'] = $wpdb->prepare($_POST['sk_recid']);
                                }
                                if(isset($_POST['sk_created'])){
                                    $updateArray['sk_created'] = $wpdb->prepare($_POST['sk_created']);
                                }
